...
| Knowledge items | *superseded in v0.18.0* | *superseded in v0.18.0* |
...
**The knowledge budget no longer keys on duration (v0.18.0).** Duration is only a proxy for how
much a video actually says: the same ten minutes can be a playlist rundown or a dense tutorial,
and a flat per-minute rule over-permits the first while truncating the second. `evidence.py
--k-budget` now measures the information actually in hand at synthesis time — transcript character
count, corrected by CJK ratio because Korean carries roughly twice the information per character,
plus the number of screen V-lines — and emits `KFLOOR n KCEILING m` for the orchestrator to inject
into the synthesis dispatch. Frame and zoom budgets remain duration-proportional as above. See
[`docs/eval/reports/2026-08-22-info-budget.md`](docs/eval/reports/2026-08-22-info-budget.md).
...
| **R14 (grounding, v0.20.0)** | Closed the gap where the pipeline **read the screen but never turned it into knowledge**: an audit of the v0.18.0 output found 0 of 60 knowledge items citing a frame, and a caption value beating the screen into the canonical record (captions said "scale it down to 30", the screen read `Size 25`, the document recorded 30). Three layers: a deterministic `screen_check` that compares numeric tokens in `setting`/`command` items against screen observations within ±90s — drawing candidates from the **raw cached transcription, not just registered observations**, since selective registration keeps only cited ones and a contradicting value is precisely what goes uncited — annotating `screen_conflict` and rendering `⚠️ 화면값 X (자막 Y)` **without ever rewriting the text**; a citation check on the synthesis reply (`V 인용 j건`) that re-synthesizes once when nothing is grounded; and a contract requiring concrete on-screen values to be registered even when captions never mention them | Frame-cited knowledge **0 → 14**, registered observations 10 (all uncited) → 33, format round-trips **1 → 0**, cost $2.568. The document now reads "Size **28px** (captions say *scale it down to 30*)" — screen adopted, disagreement stated — and screen-only settings that were previously absent entirely (`Stroke weight: Small`, `Display word: 4 words`, `Position Y 60%`) are captured. Offline precision across 7 preserved caches (pools up to 542 observations, 436 knowledge items): **1 flag, the known real error, 0 false positives** — after two tightening passes the controller caught mid-round. Residual: dense settings panels read differently run to run (the same field came out 25, 28, or unreadable), so unread values remain a structural blind spot — see [`docs/eval/reports/2026-08-22-grounding.md`](docs/eval/reports/2026-08-22-grounding.md) |
| **R13 (info-budget, v0.18.0)** | Moved the knowledge budget's anchor from **duration** to **measured information volume**: `evidence.py --k-budget` counts transcript characters (CJK-ratio language correction — Korean carries ~2× the information per character, so an uncorrected budget starves Korean videos) plus screen V-lines, emits `KFLOOR n KCEILING m`, and the orchestrator injects those numbers into the synthesis dispatch; `synthesize.md` lost its hardcoded caps and gained "the floor is a target, not a quota — if the material isn't there, fall short rather than invent". Also repaired the K/C timestamp-position variants that had rejected a whole batch on five consecutive runs, by **deriving the timestamp from the record's own refs** (cited frame's time, else the cited caption segment's start; never a fake 0.0) | Same 13-minute video: knowledge **36 → 60**, cost **$2.665 → $2.159**, **$/knowledge-item $0.074 → $0.036 (−51%, best measured)**; a 4.5-minute video got floor 16 / ceiling 32 and settled at **20 — touching neither bound**, the first measured evidence that the budget stopped being the answer; zero unfounded items in both. Open finding: frame-cited knowledge fell to 0% in the confirming run (historically 8–30%) — a possible metric-gaming effect of the new floor, queued for a grounding guard — see [`docs/eval/reports/2026-08-22-info-budget.md`](docs/eval/reports/2026-08-22-info-budget.md) |
