...
**M < N×2이면 빈약 전사** — 같은 디스패치를 1회만 재시도(지시에 '빈약 전사 감지 — 프레임당 값을 빠짐없이, 요약 금지. 읽을 텍스트가 없는 프레임은 없다고 보고하라 — 장면·사물 묘사로 줄 수를 채우지 마라' 추가). 재시도 후에도 낮으면 **이 영상은 화면에 텍스트가 없는 것**이니 그대로 진행하고(8단계 `--note "저밀도 전사 n/장"`) **4단계의 빈약 검사는 건너뛴다 — 재시도 예산은 영상당 1회다**(실측: 토킹헤드 영상이 패스마다 재시도를 태워 회당 $0.3~0.5를 헛썼고, 늘어난 줄은 "링라이트·포스터" 같은 장면 묘사였다).
...
**2단계에서 재시도를 이미 썼으면 빈약 검사를 하지 마라**(영상당 1회). 아니면 최종 응답 `프레임 N, V라인 M`에서 **M < N×2이면 빈약 전사** — 같은 디스패치를 1회만 재시도(지시 문구는 2단계와 같다). 재시도 후에도 낮으면 그대로 진행하되 8단계 `--note "저밀도 전사 n/장"`으로 보고한다.
