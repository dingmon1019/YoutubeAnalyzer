...
하면 $1.44로 추정된다. 상위 모델 세션에서 호출됐다면 사용자에게 한 줄로 알리고 그대로 진행한다 —
스킬이 세션 모델을 바꾸지는 않는다.

단 **이 절감은 아직 실현되지 않았다.** 실측(2026-08-18)에서 sonnet 본체는 headless 실행에서
서브에이전트를 백그라운드로 위임한 뒤 응답을 끝내 파이프라인이 미완주했고(video.md 미생성,
전 항목 unaudited), 완주를 강제하자 폴링으로 콜이 불어나 오히려 비쌌다. 상위 모델 세션이면
그대로 진행하고, sonnet으로 내릴 때는 **완주 여부(video.md 생성·감사 스탬프)를 직접 확인**하라.
...
- 산출 계약: `<cache_dir>/zoom-plan.json`**과 `<cache_dir>/visual-coverage.json`**을 Write한 뒤,
  **최종 응답으로 아래 4줄만** 반환
  ...
  3. `확대 배정:` 1024px N장 / 512px N장 — **`(텍스트 화면 M개 중 N개 1024)` 를 반드시 덧붙인다.**
     ...
     값을 못 읽는다는 뜻이므로, 상한(1024 8장)에 걸린 게 아니라면 N = M이 되도록 다시 배정한다.
...
**오케스트레이터는 `확대 배정` 줄의 `N < M`을 확인한다.** 상한(1024 8장) 때문이 아닌데 N이 M보다
...
6. 예산: **1024px 확대는 총 8장**, 세션 전체 **확대 프레임(재확대·Q&A 포함)은 12장**이
   **상한이다.** 상한은 목표가 아니다 — 남기려 애쓰지 말고, 규칙 5에 해당하는 화면이면
   ...
   채우고 나머지는 512로 내린 뒤 그 사실을 `특이`에 적는다. **단 규칙 5의 미관측 지점은 512로
   내리지 않는다** — 화면을 보지 못한 채 "읽을 텍스트가 없다"를 확정할 수 없기 때문이다. 미관측
   지점이 8장을 넘으면 그 사실 자체를 `특이`에 적고 상한을 지킨다.
