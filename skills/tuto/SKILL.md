...
- 산출 계약: `<cache_dir>/guide.md` 작성(감사 스탐프 자리는 "(표본 감사 후 기입)") 후 **최종 응답으로 §5 규칙에 맞는 표본 주장 후보 6건**(서식·상태 변화 ≥1건, 해당 범주 부재 시 "서식변화 주장 없음" 명시)을 주장 1문장+근거 프레임 절대경로로 반환
...
빌더가 반환한 후보 6건을 사용하되, 후보가 §5 규칙에 어긋나면 오케스트레이터가 직접 재표집한다 — 화면녹화 튜토리얼에서 서식·상태변화 주장(볼드/색상/토글/활성화)을 최소 1건 포함해야 하기 때문이다. 가이드에 해당 범주의 주장이 하나도 없으면 생략하고 스탬프에 '서식변화 주장
